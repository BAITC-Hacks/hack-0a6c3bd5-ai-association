...
Состояние реализации H3 + защиты backend H4 на 23.09.2026. Один Python-процесс, локальная SQLite и собранный React-интерфейс. Владелец backend — Даниял (A), интерфейса и документации — Мовсар (B).
...
`uv run hack serve` создаёт отсутствующую БД. Для прямого `uvicorn app.main:app` сначала требуется `python -m app.cli seed`. Compose монтирует готовый `web/dist` из клона и сохраняет БД в отдельном томе. [Команды установки](../README.md), [проверки H4](QA-H4.md), [90-секундное демо](DEMO.md).

Перед изменением интерфейса читайте [CLAUDE-HANDOFF.md](CLAUDE-HANDOFF.md): упрощение пути не должно обходить подтверждение, сбрасывать UUID при повторе или переносить деньги и корзину в локальные моки.
