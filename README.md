...
**Состояние:** H1–H3 реализованы; H4 — проверка запуска и подготовка к сдаче. Работает локальный снимок из **13 позиций: 10 карточек ekt.kz и 3 синтетические позиции**. Это не полный и не обновляемый в реальном времени каталог. Редизайн передаётся Claude по [готовому промпту](docs/CLAUDE-PROMPT.md) и [карте интерфейса](docs/CLAUDE-HANDOFF.md).

![Чат и проверка характеристик](docs/assets/kontur-chat-desktop.png)
![Проверка строк спецификации](docs/assets/kontur-upload-desktop.png)
...
Python 3.12, FastAPI, Pydantic и SQLite — один локальный процесс и файл данных. OpenAI SDK — structured outputs с таймаутом, повтором и fallback. React, TypeScript, Vite, Tailwind и собственный CSS — интерфейс без готовой UI-темы. Lucide и шрифты Manrope / IBM Plex Mono включены локально; CDN в runtime нет. Версии закреплены в `uv.lock` и `web/package-lock.json`.
...
1. Откройте `/?view=upload`, скачайте `sample.xlsx` и загрузите его. Видны **две редактируемые строки**.
2. Нажмите «Проверить по каталогу». В чате появится конфликт **160 / 250 А** у `200300285_`; предложения нет, корзина пуста.
3. Нажмите «Исправить строки», исключите вторую строку и снова запустите проверку. Появится предложение **2 шт `DEMO-160-AVAILABLE` по 61 000 ₸**, итог **122 000 ₸**; корзина ещё пуста.
4. Нажмите «Проверить и подтвердить», проверьте склад и состав, затем подтвердите в диалоге. Сервер изменит корзину.
...
Границы файлов и порядок синхронизации — [TEAM.md](docs/TEAM.md). Почасовые результаты — [PROGRESS.md](PROGRESS.md), использование Codex — [codex-log.md](docs/codex-log.md).
...
195 backend-тестов прошли при интеграции H3/H4. Playwright проверил подтверждение, повтор с прежним UUID после потери ответа, две вкладки, ошибки файлов, все четыре образца и экран 390 px. Чистый клон macOS прошёл сценарий без внешних запросов; сборка и образцы побайтово воспроизводимы. Полная карта для следующей работы над UI — [CLAUDE-HANDOFF.md](docs/CLAUDE-HANDOFF.md).
