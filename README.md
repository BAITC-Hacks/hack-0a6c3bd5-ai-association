...
Материалы защиты: [видео демо — 76 секунд](docs/assets/kontur-demo.mp4), [русские субтитры](docs/assets/kontur-demo.srt), [презентация из шести слайдов](docs/pitch.html), [речь для двух участников и ответы жюри](docs/PITCH.md). Видео без озвучки; встроенные субтитры можно включить в проигрывателе. HTML-презентация использует локальные ресурсы из клона; управление стрелками и пробелом. [Проверки H5](docs/QA-H5.md), [порядок сдачи](docs/SUBMISSION.md).
...
- Запуск на Windows 11 подтверждён командой через Мовсара; Codex в этом окружении проверял macOS. Живой OpenAI/OCR и Docker остаются непроверенными. Актуальные результаты и оставшиеся пункты — [QA-H5.md](docs/QA-H5.md).
